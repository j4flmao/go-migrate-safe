...
func ListMyOrders(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := db.QueryContext(c.Request.Context(),
			"SELECT id, status, total, created_at FROM orders WHERE user_id = ? ORDER BY created_at DESC", uid)
		if err != nil {
...
func GetOrder(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := db.QueryRowContext(c.Request.Context(),
			"SELECT id, user_id, status, total, created_at FROM orders WHERE id = ?", c.Param("id"),
...
func CreateOrder(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for i, it := range in.Items {
			var info itemInfo
			err := db.QueryRowContext(c.Request.Context(),
				"SELECT price, stock FROM products WHERE id = ?", it.ProductID,
		...
		for i, it := range in.Items {
			...
			_, err = tx.ExecContext(c.Request.Context(),
				"UPDATE products SET stock = stock - ? WHERE id = ?", it.Quantity, it.ProductID)
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
				return
			}
