...
func SeedRun(dsn string) {
	...
	if _, err := db.ExecContext(ctx, "SELECT setval(pg_get_serial_sequence('products','id'), (SELECT MAX(id) FROM products))"); err != nil {
		log.Printf("seed: reset products sequence: %v", err)
	}
	if _, err := db.ExecContext(ctx, "SELECT setval(pg_get_serial_sequence('users','id'), (SELECT MAX(id) FROM users))"); err != nil {
		log.Printf("seed: reset users sequence: %v", err)
	}
	if _, err := db.ExecContext(ctx, "SELECT setval(pg_get_serial_sequence('categories','id'), (SELECT MAX(id) FROM categories))"); err != nil {
		log.Printf("seed: reset categories sequence: %v", err)
	}
