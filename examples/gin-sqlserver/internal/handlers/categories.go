...
func GetCategory(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := db.QueryRowContext(c.Request.Context(),
			"SELECT id, name, slug, description, created_at FROM categories WHERE id = @p1", c.Param("id"),
...
func CreateCategory(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var id int64
		err := db.QueryRowContext(c.Request.Context(),
			"INSERT INTO categories (name, slug, description) VALUES (@p1, @p2, @p3); SELECT SCOPE_IDENTITY()",
